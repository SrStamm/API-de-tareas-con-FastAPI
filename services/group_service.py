...
class GroupService:
    ...
    async def get_groups_with_cache(self, limit: int, skip: int) -> List[ReadGroup]:
        ...
        to_cache = [
            {**group.model_dump(), "users": [user.model_dump() for user in group.users]}
    ...
    async def create_group(self, new_group: CreateGroup, user_id: int):
        try:
            ...
            try:
                ...
                logger.info(
                    f"[create_group] Cache Delete - Key: groups:limit:*:offset:*"
                )
            except redis.RedisError as e:
    ...
    async def update_group(
        ...
        try:
            ...
            try:
                await redis_client.delete(f"groups:limit:*:offset:*")
                logger.info(
                    f"[update_group] Cache Delete - Key: groups:limit:*:offset:*"
                )
            except redis.RedisError as e:
    ...
    async def delete_group(self, group_id: int):
        try:
            ...
            try:
                await redis_client.delete(f"groups:limit:*:offset:*")
                logger.info(
                    f"[delete_group] Cache Delete - Key: groups:limit:*:offset:*"
                )
            except redis.RedisError as e:
    ...
    async def append_user(self, group_id, user_id: int, actual_user_id: int):
        try:
            ...
            try:
                ...
                logger.info(
                    f"[append_user_group] Cache Delete - Key: groups:user_id:{actual_user_id}:limit:*:offset:*"
                )
            except redis.RedisError as e:
                logger.warning(
                    f"[append_user_group] Cache Delete Error | Error: {str(e)}"
                )
            ...
            if user in group.users:
                ...
                if (
                    ...
                    try:
                        ...
                        logger.info(
                            f"[delete_user] Cache Delete - Key: groups:user_id:{actual_user_id}:limit:*:offset:*"
                        )
                    except redis.RedisError as e:
                        logger.warning(
                            f"[delete_user] Cache Delete Error | Error: {str(e)}"
                        )
    ...
    async def update_user_role(
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"[get_groups] Cache Delete - Key: groups:user_id:{actual_user_id}:limit:*:offset:*"
                )
            except redis.RedisError as e:
