...
from conftest import auth_headers, client, test_create_project_init, select, db_models, async_client
...
@pytest.mark.asyncio
async def test_send_message_to_project(async_client, auth_headers, test_create_project_init):
    response = await async_client.post('/chat/1', headers=auth_headers, json={'content':'Enviando mensaje por http'})
