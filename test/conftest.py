...
def test_user(test_session):
    user = db_models.User(
        username="mirko", email="mirko@dev.com", password=encrypt_password(PASSWORD)
    )
...
def test_user2(test_session):
    user = db_models.User(
        username="moure", email="moure@dev.com", password=encrypt_password(PASSWORD)
    )
    test_session.add(user)
    test_session.commit()
...
def test_user3(test_session):
    user = db_models.User(
        username="dalto", email="dalto@dev.com", password=encrypt_password(PASSWORD)
    )
    test_session.add(user)
    test_session.commit()
...
async def test_create_project_init(
    async_client,
    auth_headers,
    test_user2,
    test_user3,
    ...
    # Crear proyecto
    response = await async_client.post(
        "/project/1", headers=auth_headers, json={"title": "creando un proyecto"}
    )
...
@pytest.fixture
async def test_create_project_init_for_tasks(
    async_client,
    auth_headers,
    test_user2,
    test_user3,
