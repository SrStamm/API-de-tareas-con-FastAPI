...
async def test_get_projects_iam_error(mocker):
    ...
    mock_project_repo.get_all_project_by_user.side_effect = DatabaseError(
        SQLAlchemyError("db error"), "get_all_project_by_user"
    )
...
async def test_get_all_projects_error(mocker):
    ...
    mock_project_repo.get_all_projects.side_effect = DatabaseError(
        SQLAlchemyError("db error"), "get_all_projects"
    )
...
async def test_get_users_in_project_error(mocker):
    ...
    mock_project_repo.get_users_in_project.side_effect = DatabaseError(
        SQLAlchemyError("db error"), "get_users_in_project"
    )
...
@pytest.mark.asyncio
async def test_get_users_in_project_error_user_not_found_error(mocker):
    mock_project_repo = mocker.Mock()
    mock_group_serv = mocker.Mock()
    mock_user_serv = mocker.Mock()
    ...
    mock_project_repo.get_users_in_project.return_value = None
    with pytest.raises(UsersNotFoundInProjectError):
        await service.get_user_in_project(1, 1, 1, 1)
...
async def test_create_project_error(mocker):
    ...
    mock_project_repo.create.side_effect = DatabaseError(
...
async def test_update_project_error(mocker):
    ...
    mock_project_repo.update.side_effect = DatabaseError(
        SQLAlchemyError("db error"), "update_project"
    )
...
async def test_delete_project_error(mocker):
    ...
    mock_project_repo.delete.side_effect = DatabaseError(
        SQLAlchemyError("db error"), "delete_project"
    )
...
async def test_add_user_error(mocker):
    ...
    mock_project_repo.add_user.side_effect = DatabaseError(
        SQLAlchemyError("db error"), "add_user"
    )
...
async def test_remove_user_error(mocker):
    ...
    mock_project_repo.remove_user.side_effect = DatabaseError(
        SQLAlchemyError("db error"), "remove_user"
    )
...
async def test_update_user_permission_in_project_error(mocker):
    ...
    mock_project_repo.update_permission.side_effect = DatabaseError(
        SQLAlchemyError("db error"), "update_user_permission_in_project"
    )
